(function () {
  ...
  const liveShareStatus = document.getElementById('liveShareStatus');
  ...
  // Tells the person, right where they're about to check the box,
  // whether "shared" actually means "live for everyone right now" on
  // this deployment or "saved here until manually exported" — instead
  // of letting the label promise something the current setup can't do.
  function renderLiveShareStatus() {
    const live = C.isSharedSyncLive();
    liveShareStatus.style.color = live ? 'var(--accent)' : 'var(--text-faint)';
    liveShareStatus.textContent = T(live ? 'collect.liveStatus.on' : 'collect.liveStatus.off');
  }
  renderLiveShareStatus();
